...
{
    TFTLCD->LCD_DATA=~color;	//取反写入，修正LCD颜色反转
}
...
void LCD_ShowFontHZ(u16 x, u16 y, u8 *cn)
{
	u8 i, j, k;
	u8 dat;
	u8 num = sizeof(CnChar16x16) / 32;

	while (*cn != '\0')
	{
		for (i=0; i<num; i++)
		{
			if (HZ_Chars[i*3]==cn[0] && HZ_Chars[i*3+1]==cn[1] && HZ_Chars[i*3+2]==cn[2])
			{
				...
				for (j=0; j<32; j++)
				{
					...
					for (k=0; k<8; k++)
					{
						if (dat & 0x80) LCD_WriteData_Color(FRONT_COLOR);
						else LCD_WriteData_Color(BACK_COLOR);
						dat <<= 1;
					}
				}
				break;
